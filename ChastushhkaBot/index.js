	...
	bot.on('message', async msg => {
		...
		try {
			...
			if (text === '/info') {
				await bot.sendMessage(
					chatId,
					`Твое имя ${msg.from.first_name} и твой username: ${msg.from.username}`
				);
				return;
			}
			...
			if (text.length < 100) {
				await bot.sendMessage(
					chatId,
					'Твой текст слишком короткий. Минимальная длина текста - 100 символов'
				);
				return;
			}
			if (text.length > 120) {
				await bot.sendMessage(
					chatId,
					'Твой текст слишком длинный. Максимальная длина текста - 120 символов.'
				);
				return;
			}
			...
			await new Promise((resolve, reject) => {
				new gTTS(text, 'ru').save(audioFile, err =>
